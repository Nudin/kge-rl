...
class SGD(object):
    ...
    def max_margin(self, scores):
        ...
        return loss / (scores.size()[1] - 1.0)

    def logistic(self, scores):
        y_pos = util.to_var(
            np.ones(scores.size()[0], dtype="float32"), requires_grad=False
        )
        y_neg = util.to_var(
            np.zeros(scores.size()[0], dtype="float32"), requires_grad=False
        )
    ...
    def save(self, dump=False):
        ...
        if self.evaluator.comparator(curr_score, self.prev_score) or dump:
            ...
            torch.save(
                ...
                os.path.join(self.results_dir, "{}_params.pt".format(self.model_name)),
            )
            # Save Optimizer Gradient History for resuming training
            state_path = os.path.join(
                self.results_dir, "{}_optim_state.pt".format(self.model_name)
            )
...
class Reinforce(SGD):
    ...
    def sample(self, batch, is_target, scores, entities):
        ...
        for count in range(policy_np.shape[0]):
            ...
            proj_policy = self.project_policy(
                batch[count], policy_np[count], neg_map, is_target
            )
            ...
            rewards = self.compute_reward(
                samples,
                samples_idx,
                scores[count, 0].data.cpu().numpy(),
                scores[count, 1:].data.cpu().numpy(),
            )
            if len(rewards.keys()) > 0:
                for ind, s in enumerate(rewards.keys()):
                    assert s not in positives
                    loss += policy[count, neg_map[s]] * Variable(
                        torch.from_numpy(np.asarray([rewards[s]], dtype="float32")),
                        requires_grad=False,
                    )
        return loss

    def compute_reward(self, actions, actions_idx, pos_score, scores):
        scores_map = {a: -1.0 * scores[actions_idx[i]] for i, a in enumerate(actions)}
