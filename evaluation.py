...
class RankEvaluator(Evaluator):
    ...
    def evaluate(self, batch):
        ...
        s_mrr = np.mean([1.0 / r for r in s_rank])
        t_mrr = np.mean([1.0 / r for r in t_rank])
        return (s_mrr + t_mrr) / 2.0
...
class TestEvaluator(Evaluator):
    ...
    def metrics(self, batch, is_target):
        ...
        scores = (
            self.model.predict(batch, negs, is_target, is_pad=True).data.cpu().numpy()
        )
    ...
    def evaluate(self, batch):
        ...
        return (rr_s + rr_t) / 2.0, (hits_s + hits_t) / 2.0
