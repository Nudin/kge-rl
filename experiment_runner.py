...
from evaluation import RankEvaluator, TestEvaluator
...
def main(exp_name, data_path, resume, tune, vectors):
    ...
    if vectors:
        ...
        config = json.load(
            open(
                os.path.join(
                    data_path, "{}".format(exp_name), "config.json".format(exp_name)
                )
            )
        )
        print("Saving Embeddings")
        save_embeddings(
            os.path.join(data_path, exp_name), config["model"], is_cpu=not cuda
        )
        print("Embeddings Saved.")
        exit(0)
    config = json.load(
        open(os.path.join(data_path, "experiment_specs", "{}.json".format(exp_name)))
    )
...
def train(config, exp_name, data_path, resume=False, tune=False):
    ...
    data_set = data_loader.read_dataset(
        data_path, results_dir, dev_mode=True, max_examples=float("inf")
    )
    cuda = torch.cuda.is_available()
    ...
    model, neg_sampler, evaluator = build_model(
        data, config, results_dir, data_set["num_ents"], data_set["num_rels"]
    )
...
def evaluate(data, evaluater, results_dir, is_dev, filtered):
    ...
    for count, d in enumerate(util.chunk(data, constants.test_batch_size)):
        ...
        if count % report_period == 0:
            ...
            secs = end - start
            speed = "Speed {} queries per second".format(
...
def build_model(
    triples, config, results_dir, n_ents, n_rels, train=True, filtered=True
):
    def get_model():
        if config["model"] == "rescal":
            ...
            raise NotImplementedError(
                "Model {} not implemented".format(config["model"])
            )

    def get_neg_sampler(model=None):
        if not train:
            return negative_sampling.Random_Sampler(
                triples, float("inf"), filtered=filtered
            )
            ...
            return negative_sampling.Typed_Sampler(
                triples, config["num_negs"], results_dir
            )
            ...
            raise NotImplementedError(
                "Neg. Sampler {} not implemented".format(config["neg_sampler"])
            )
