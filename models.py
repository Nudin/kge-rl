...
class KGE(nn.Module):
    ...
    def predict(self, batch, negs=None, is_target=True, is_pad=False):
        sources, targets, rels = util.get_triples(
            batch, negs, is_target=is_target, volatile=True, is_pad=is_pad
        )
...
class Rescal(KGE):
    def __init__(self, n_ents, n_rels, ent_dim):
        super(Rescal, self).__init__(
            n_ents, n_rels, ent_dim, ent_dim * ent_dim, max_norm=True
        )
        self.dim = ent_dim
    ...
    def init(self):
        ...
        self.entities.weight.data.uniform_(-1.0, 1.0)
        self.rels.weight.data.uniform_(-1.0, 1.0)
...
class ComplEx(KGE):
    ...
    def complex(self, sources, rels, targets):
        ...
        out = (
            self.inner_prod(targets, rels, sources)
    ...
    def output(self, entities, rels, is_target):
        ...
        if is_target:
            out = (
                torch.mul(entities, rels)
                + torch.mul(entities_i, rels)
                + torch.mul(rels_i, entities)
                - torch.mul(rels_i, entities_i)
            )
        else:
            out = (
                torch.mul(entities, rels)
