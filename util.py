...
def get_triples(batch, negs, is_target=True, volatile=False, is_pad=False):
    ...
    return (
        to_var(sources, volatile=volatile, requires_grad=False),
        to_var(targets, volatile=volatile, requires_grad=False),
        to_var(rels, volatile=volatile, requires_grad=False),
    )
...
def to_var(x, volatile=False, requires_grad=False):
    ...
    cuda = cache["cuda"]
    var = Variable(
        torch.from_numpy(np.asarray(x)), volatile=volatile, requires_grad=requires_grad
    )
    if cuda:
        return var.cuda()
    return var
