...
class Negative_Sampler(object):
    ...
    def filter_candidates(self, ex, is_target, candidates):
        if self.filtered:
            known_candidates = (
                self.t_filter[(ex.s, ex.r)]
                if is_target
                else self.s_filter[(ex.r, ex.t)]
            )
...
class Random_Sampler(Negative_Sampler):
    def __init__(self, triples, num_samples, filtered=False):
        ...
        print(
            "\tNeg. Sampler: Random, num_samples: {}, filtered: {}".format(
                num_samples, filtered
            )
        )
...
class Static_Sampler(Negative_Sampler):
    ...
    def batch_sample(self, batch, is_target, num_samples=0):
        batched_negs = [
            self.sample(ex, is_target, self.get_candidates(ex, is_target), num_samples)
            for ex in batch
        ]
        return batched_negs
...
class Corrupt_Sampler(Static_Sampler):
    def __init__(self, triples, num_samples, filtered=True):
        ...
        print(
            "\tNeg. Sampler: Corrupt, num_samples: {}, filtered: {}".format(
                num_samples, filtered
            )
        )
    ...
    def get_candidates(self, ex, is_target):
        return (
            self._typed_entities[ex.r][1]
            if is_target
            else self._typed_entities[ex.r][0]
        )
...
class Typed_Sampler(Static_Sampler):
    def __init__(self, triples, num_samples, results_dir, filtered=True):
        ...
        print(
            "\tNeg. Sampler: Typed, num_samples: {}, filtered: {}".format(
                num_samples, filtered
            )
        )
...
class Relational_Sampler(Static_Sampler):
    def __init__(self, triples, num_samples, filtered=True):
        ...
        print(
            "\tNeg. Sampler: Relational, num_samples: {}, filtered: {}".format(
                num_samples, filtered
            )
        )
...
class Dynamic_Sampler(Negative_Sampler):
    ...
    def sample(self, ex, is_target, entity_vector, num_samples=0):
        ...
        while True:
            ...
            ind = self.tree.query(
                [entity_vector], k=curr_num + diff, return_distance=False
            )
            ind = ind[0, curr_num:]
...
class NN_Sampler(Dynamic_Sampler):
    def __init__(self, triples, num_samples, filtered=True):
        ...
        print(
            "\tNeg. Sampler: Nearest Neighbors, num_samples: {}, filtered: {}".format(
                num_samples, filtered
            )
        )
    ...
    def batch_sample(self, batch, is_target, num_samples=0):
        ...
        batched_negs = [
            self.sample(ex, is_target, e_v, num_samples)
            for ex, e_v in zip(batch, entity_vectors)
        ]
        return batched_negs
...
class Adversarial_Sampler(Dynamic_Sampler):
    def __init__(self, triples, num_samples, filtered=True):
        ...
        print(
            "\tNeg. Sampler: Adversarial, num_samples: {}, filtered: {}".format(
                num_samples, filtered
            )
        )
...
class Policy_Sampler(Random_Sampler):
    def __init__(self, triples, num_samples, filtered=True):
        ...
        print(
            "\tNeg. Sampler: Policy Gradient, Num. Samples {}".format(
                self.num_samples
    ...
    def sample_arms(self, arms, prob, ex, is_target):
        ...
        if np.random.uniform() <= self.eps:
            return self.sample(
                ex, is_target, num_samples=min(len(arms), self.min_samples)
            )
    ...
    def pos(self, ex, is_target):
        return list(
            self.t_filter[(ex.s, ex.r)] if is_target else self.s_filter[(ex.r, ex.t)]
        )
